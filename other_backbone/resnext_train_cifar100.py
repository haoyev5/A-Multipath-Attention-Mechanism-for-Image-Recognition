...
print('test complete')
'''
pretrained_net = model_zoo.get_model(name='resnext50_32x4d', ctx=ctx, pretrained=True)
#net = model_zoo.get_model(name='resnext50_32x4d', ctx=ctx, pretrained=False, classes=num_classes)
#net.initialize(ctx=ctx, init=init.Xavier())

net = nn.HybridSequential()
net.add(pretrained_net.features[0],
        unit_split.Attention(64),
        pretrained_net.features[1],        
        pretrained_net.features[2],
        pretrained_net.features[3],
        pretrained_net.features[4],
        unit_split.Attention(256),
...
X = nd.random.uniform(shape=(128,3,32,32)).as_in_context(ctx)
for layer in net:
    X = layer(X)
    print(layer.name, 'output shape:\t', X.shape)
print('test complete')    
